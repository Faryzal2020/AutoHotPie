...
pie_sendKeyWithHeldModifiers(keyObject)
{
	...
	if (InStr(heldModifiers, "#")) {
		send, {LWin down}
		OutputDebug, MODIFIER: Win DOWN
	}
	if (InStr(heldModifiers, "+")) {
		send, {shift down}
		OutputDebug, MODIFIER: Shift DOWN
	}
	if (InStr(heldModifiers, "^")) {
		send, {ctrl down}
		OutputDebug, MODIFIER: Ctrl DOWN
	}
	if (InStr(heldModifiers, "!")) {
		send, {alt down}
		OutputDebug, MODIFIER: Alt DOWN
	}
	...
	if (InStr(heldModifiers, "!")) {
		send, {alt up}
		OutputDebug, MODIFIER: Alt UP
	}
	if (InStr(heldModifiers, "^")) {
		send, {ctrl up}
		OutputDebug, MODIFIER: Ctrl UP
	}
	if (InStr(heldModifiers, "+")) {
		send, {shift up}
		OutputDebug, MODIFIER: Shift UP
	}
	if (InStr(heldModifiers, "#")) {
		send, {LWin up}
		OutputDebug, MODIFIER: Win UP
	}
