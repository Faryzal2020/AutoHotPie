...
var editPieMenu = {
        ...
        function handleMouseUp(mouseEvent){
            ...
            if(draggingElement && draggingElement.type == "sliceLabel"){
                ...
                if(clickedElement.selectable == true){
                    ...
                    } else if(clickedElement.type == "duplicateSliceBtn"){
                        editPieMenu.slice.duplicate(clickedElement.data)
                    }                    
                }
            }
            clickedElement = null;
    ...
    slice:{        
        ...
        duplicate: function(slice=null){
            let duplicateIndex
            if (slice == null){
                duplicateIndex = editPieMenu.selectedPieMenu.functions.indexOf(editPieMenu.selectedSlice);
            }else{
                duplicateIndex = slice;                
            }
    ...
    pieMenuDisplay:{
        ...
        draw:{
            ...
            elements:function(elements){
                ...
                for (k in elements){
                    ...
                    } else if(element.type == "duplicateSliceBtn"){
                        element.region={rect:[
                            selectedSliceButtonPosition[0]-editButtonSize,
                            selectedSliceButtonPosition[1]+(editButtonSize*2)+(buttonPadding*2),
                            editButtonSize,
                            editButtonSize],arc:null}
                        disp.draw.duplicateSliceButton([selectedSliceButtonPosition[0]-editButtonSize,selectedSliceButtonPosition[1]+(editButtonSize*2)+(buttonPadding*2),editButtonSize,editButtonSize])
                    }
                };                 
            ...
            duplicateSliceButton:function(rect){
                let ctx = editPieMenu.pieMenuDisplay.activeCanvas.getContext("2d");
                let rectCenter = [rect[0]+(rect[2]/2),rect[1]+(rect[3]/2)];
                let iconPadding = 5;
                ...
                // Draw duplicate icon (two overlapping rectangles) - made larger and more visible
                ctx.strokeStyle = 'white';
                ctx.lineWidth = 2;
                
                // Calculate icon size based on button size
                let iconSize = Math.min(rect[2], rect[3]) * 0.3; // 30% of button size
                let iconSpacing = iconSize * 0.3; // Spacing between rectangles
                
                // First rectangle
                let rect1X = rectCenter[0] - iconSize - (iconSpacing / 2);
                let rect1Y = rectCenter[1] - (iconSize / 2);
                ctx.strokeRect(rect1X, rect1Y, iconSize, iconSize);
                
                // Second rectangle (slightly offset)
                let rect2X = rectCenter[0] + (iconSpacing / 2);
                let rect2Y = rectCenter[1] - (iconSize / 2);
                ctx.strokeRect(rect2X, rect2Y, iconSize, iconSize);
            }       
        },
        refresh:function(){                       
            editPieMenu.pieMenuDisplay.loadPieMenuElements(editPieMenu.selectedPieMenu);
        ...
        getElementUnderMouse: function(mouse){
            ...
            // First pass: check for buttons (add, duplicate, delete) - prioritize these
            for (let k in disp.elements){
                let element = disp.elements[k];
                if (element.region.rect != null && 
                    (element.type === "addSliceBtn" || element.type === "duplicateSliceBtn" || element.type === "deleteSliceBtn")){                    
            ...
            // Second pass: check for other elements
            for (let k in disp.elements){
                let element = disp.elements[k];
                if (element.region.rect != null){                    
                    if(element.region.rect[0] < mouse.position[0] && mouse.position[0] < (element.region.rect[0]+element.region.rect[2]) ){
    ...
    sliceSettings:{
        ...
        sliceFunction: {
            ...
            sendKeyWithHeldModifiers:{
                keysDiv: document.getElementById('send-keys-with-held-modifiers-div'),
                keysListDiv: $('#send-key-with-held-modifiers-button-list'),
                keyButtonGroupTemplate: document.getElementById('send-key-btn-group-template'),               
                keyButtonTemplate: document.getElementById('send-key-btn-template'),               
                keyAddButton: $('#send-key-with-held-modifiers-add-keystroke-btn'),
                timeBetweenKeysDiv: document.getElementById('time-between-keys-with-held-modifiers-input-div'),
                modifierHoldDelayDiv: document.getElementById('modifier-hold-delay-input-div'),
                heldModifiersDiv: document.getElementById('held-modifiers-checkbox-group')
            },
            mouseClick:{
        ...
        initialize:function(){   
                ...
                function processFunctionSelection(functionNameString, configureFunctionOnSelect=false){
                    ...
                    function checkForDefaultLabel(){                        
                        ...
                        case "openSettings":
                            ...
                            return;
                        case "sendKeyWithHeldModifiers":
                            setDefaultLabel("Send Key with Held Modifiers","SendKey.png");
                            return;
                        default:
                            return;
                        }
            ...
            // Send Key with Held Modifiers
            this.sliceFunction.sendKeyWithHeldModifiers.keysDiv.addEventListener("click",function(event){
                selKeyObj = editPieMenu.selectedSlice.params
                if(!event.target){
                    return;
                }
                ...
                if(target.name && target.name.slice(0,23) == "held-send-keystroke-btn"){
                    //Get index
                    let keyIndex = parseInt(target.name.split("-").slice(-1)[0])
                    //Remove or edit
                    
                    if(target.name.split("-")[4]=="remove"){
                        selKeyObj.keys.splice(keyIndex,1)
                        editPieMenu.sliceSettings.loadSelectedPieKey();
                    }else if(target.name.split("-")[4]=="add"){
                        assignKey().then(val => { 
                            selKeyObj.keys.push(val.ahkKey)                            
                            $('[href="#tab-2"]').tab('show');
                            window.scrollTo(0,document.body.scrollHeight);
            ...
            this.sliceFunction.sendKeyWithHeldModifiers.timeBetweenKeysDiv.addEventListener('mousedown', (event) => {
                let newValue = handleSliderDiv(event);
                (typeof(newValue) === 'number') && (editPieMenu.selectedSlice.params.keyDelay = newValue)
                editPieMenu.pieMenuDisplay.refresh();
            });
            this.sliceFunction.sendKeyWithHeldModifiers.timeBetweenKeysDiv.addEventListener('mousemove', (event) => {
                let newValue = handleSliderDiv(event);
                (typeof(newValue) === 'number') && (editPieMenu.selectedSlice.params.keyDelay = newValue)
                editPieMenu.pieMenuDisplay.refresh();
            });
            this.sliceFunction.sendKeyWithHeldModifiers.timeBetweenKeysDiv.addEventListener('change', (event) => {
                let newValue = handleSliderDiv(event);
                (typeof(newValue) === 'number') && (editPieMenu.selectedSlice.params.keyDelay = newValue)
                editPieMenu.pieMenuDisplay.refresh();
            });

            this.sliceFunction.sendKeyWithHeldModifiers.modifierHoldDelayDiv.addEventListener('mousedown', (event) => {
                let newValue = handleSliderDiv(event);
                (typeof(newValue) === 'number') && (editPieMenu.selectedSlice.params.modifierHoldDelay = newValue)
                editPieMenu.pieMenuDisplay.refresh();
            });
            this.sliceFunction.sendKeyWithHeldModifiers.modifierHoldDelayDiv.addEventListener('mousemove', (event) => {
                let newValue = handleSliderDiv(event);
                (typeof(newValue) === 'number') && (editPieMenu.selectedSlice.params.modifierHoldDelay = newValue)
                editPieMenu.pieMenuDisplay.refresh();
            });
            this.sliceFunction.sendKeyWithHeldModifiers.modifierHoldDelayDiv.addEventListener('change', (event) => {
                let newValue = handleSliderDiv(event);
                (typeof(newValue) === 'number') && (editPieMenu.selectedSlice.params.modifierHoldDelay = newValue)
                editPieMenu.pieMenuDisplay.refresh();
            });
            ...
            this.sliceFunction.sendKeyWithHeldModifiers.heldModifiersDiv.addEventListener('click', function(event) {
        ...
        loadSelectedPieKey:function(){            
            ...
            switch (selectedFunc){
                ...
                function addHeldModifiersKeystrokeButtonGroup(hotkeyObj, index, isRemoveable=true){
                    let btnClone
                    if (isRemoveable){
                        btnClone = editPieMenu.sliceSettings.sliceFunction.sendKeyWithHeldModifiers.keyButtonGroupTemplate.cloneNode(true); 
                        btnClone.children[0].setAttribute('name','held-send-keystroke-btn-' + index)
                        btnClone.children[0].innerHTML = hotkeyObj.displayKey;
                        btnClone.children[1].setAttribute('name','held-send-keystroke-btn-remove-' + index);                       
                    } else {
                        btnClone = editPieMenu.sliceSettings.sliceFunction.sendKeyWithHeldModifiers.keyButtonTemplate.cloneNode(true);
                        btnClone.setAttribute('name','held-send-keystroke-btn-' + index)
                        btnClone.innerHTML = hotkeyObj.displayKey;
                    }
                    
                    editPieMenu.sliceSettings.sliceFunction.sendKeyWithHeldModifiers.keysListDiv.append(btnClone);
                }
                    ...
                    break;
                case "Send Key with Held Modifiers":
                    ahkParamObj = editPieMenu.selectedSlice.params;
                    editPieMenu.sliceSettings.sliceFunction.sendKeyWithHeldModifiers.keysListDiv.empty();
                    
                    if (ahkParamObj.keys.length == 0){
                        $('#send-key-with-held-modifiers-assign-key-btn').show();
                        editPieMenu.sliceSettings.sliceFunction.sendKeyWithHeldModifiers.keyAddButton.hide();                      
                    } else {
                        $('#send-key-with-held-modifiers-assign-key-btn').hide();
                        editPieMenu.sliceSettings.sliceFunction.sendKeyWithHeldModifiers.keyAddButton.show();                      
                    }
                    ahkParamObj.keys.forEach(function(val, index){
                        let p_HotkeyObj = new Hotkey(val);
                        if(ahkParamObj.keys.length == 1){
                            addHeldModifiersKeystrokeButtonGroup(p_HotkeyObj,index, false);
                        } else {                              
                            addHeldModifiersKeystrokeButtonGroup(p_HotkeyObj,index)
                        }                        
                    });
                    setSliderDivValue(editPieMenu.sliceSettings.sliceFunction.sendKeyWithHeldModifiers.timeBetweenKeysDiv, editPieMenu.selectedSlice.params.keyDelay, 0, 200)
                    setSliderDivValue(editPieMenu.sliceSettings.sliceFunction.sendKeyWithHeldModifiers.modifierHoldDelayDiv, editPieMenu.selectedSlice.params.modifierHoldDelay, 0, 200)
...
function setSliderDivValue(sliderDivElement,value,min,max,decimalStepDigit=0){                
    ...
    let domElement = sliderDivElement;
    if (sliderDivElement.jquery || sliderDivElement instanceof jQuery) {
        domElement = sliderDivElement[0];
    }
    
    let sliderRangeInput = domElement.getElementsByClassName('form-range')[0];
    sliderRangeInput.setAttribute('min',min);
    sliderRangeInput.setAttribute('max',max);
    sliderRangeInput.setAttribute('step', decimalStepDigit);
    sliderRangeInput.value = value*Math.pow(10,decimalStepDigit).toString();
    
    let sliderTextInput = domElement.getElementsByClassName('bg-dark border rounded-0 border-dark')[0];
